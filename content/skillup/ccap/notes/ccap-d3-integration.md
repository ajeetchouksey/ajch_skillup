...
> **Scenario.** A financial services company is building an internal agent for its claims-adjustment team. The agent needs to: (1) look up policy details from a structured claims database (exact lookups by policy number), (2) search a 40,000-document corpus of historical claim adjudication notes and internal guidelines (unstructured, natural-language), (3) call an external fraud-scoring API operated by a third-party vendor, and (4) hand off complex disputed claims to a specialized "appeals agent" that runs its own multi-step investigation workflow. The claims team has a 5-second latency budget for the initial triage response, but appeals can run asynchronously over minutes.
>
> **Reasoning it through:**
> - *Policy lookup (1)* is a structured, exact-match query against a system of record. Per the retrieval-strategy-matching principle, this is a direct API/database call, not a vector search — using RAG here would be slower and less accurate for a deterministic lookup.
> - *Historical notes search (2)* is unstructured prose with natural-language queries ("has this pattern of claim been disputed before"). This is genuine RAG territory: chunk the notes at a semantic/paragraph boundary (adjudication notes have natural argument units, not fixed 512-token boundaries), index with hybrid dense+sparse (adjusters will search both by concept and by exact claim-type codes), and tag each chunk with metadata (claim category, date, region) to allow filtered retrieval — recent guidelines should outrank a superseded 2019 policy.
> - *Fraud-scoring API (3)* is a fixed, single-purpose, external, well-known endpoint called the same way every time. This is a direct API integration, not MCP — there's no discovery benefit (the endpoint never changes) and no reuse benefit worth the protocol overhead for one vendor call in a latency-sensitive triage path.
> - *Appeals handoff (4)* requires independent multi-step reasoning owned by a distinct capability domain, with no shared latency budget — this is genuine agent-to-agent delegation, not a tool call, because the appeals agent needs to plan, gather additional evidence, and reason over a multi-step investigation on its own.
> - *Tool exposure*: the triage agent should only see tools 1–3 in its active context (least-privilege scoping) — it does not need the appeals agent's internal tools loaded, and the appeals agent, once invoked, operates with its own separately-scoped tool set. This is progressive discovery at the agent level: the triage agent doesn't need to know *how* appeals investigation works, only that it can delegate to it.
> - *Auth*: the claims database and fraud API calls must be authorized per the individual adjuster's entitlements (some adjusters can't view claims above a dollar threshold), not a single shared service credential — otherwise a junior adjuster's agent session could retrieve claims data the adjuster is not personally authorized to see.
> - *Latency budget*: the 5-second triage SLA means the RAG retrieval step for historical notes should skip an expensive reranking pass (a lighter hybrid-search-only configuration) during initial triage, while the asynchronous appeals workflow, unconstrained by that budget, can afford full reranking and extended thinking for deeper analysis.
> - *Observability*: because a wrong policy lookup or a stale guideline citation is a compliance-relevant failure, this system needs output-quality sampling (citation-grounding checks on the RAG answers) and drift monitoring on retrieval hit rates, not just uptime monitoring — a "successful" tool call that returns the wrong policy version would otherwise go undetected.
