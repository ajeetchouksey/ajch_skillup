...
> **Scenario.** A mid-size insurance company asks an architect to "build something with Claude to help our claims adjusters work faster." Six months later, the system needs to be handed off to the internal platform team, who will own it going forward. Walk the lifecycle.
>
> **Reasoning it through:**
> - *Discovery.* The architect runs structured interviews with adjusters (not just their manager) and shadows the current claims-review process, discovering the actual bottleneck isn't drafting claim summaries (what the initial request implied) but cross-referencing policy documents against claim details — a retrieval problem, not a drafting problem. The discovery brief captures this reframed objective, a baseline metric (average adjuster time per claim, currently 22 minutes), a target (a stated goal from the business sponsor of a 30% reduction), the data landscape (policy documents live in three different systems, two of which have API access and one of which is PDF-only with no API), and the stakeholder map (the claims-ops director has sign-off authority; adjusters have strong opinions but not veto power; legal has a hard constraint that no claim-approval decision can be automated). Stakeholders sign off on this brief before design starts.
> - *Design.* The architect evaluates two options: a RAG system over the two API-accessible sources only (faster to build, incomplete coverage) versus a RAG system with an added document-ingestion pipeline for the PDF-only source (slower to build, complete coverage). The ADR names the trade-off explicitly: option one ships in 6 weeks but only closes an estimated 18% of the time gap because the PDF-only carrier represents a large share of claims; option two ships in 11 weeks but is projected to close closer to the full 30% target. The claims-ops director, given both numbers, chooses option two — a decision the architect could not have made unilaterally, because it trades timeline for outcome in a way only the business sponsor should own.
> - *Handoff.* Before transferring ownership, the architect produces three documents at three altitudes: an architecture overview (for the platform team's engineering lead, showing the retrieval pipeline, the ingestion pipeline, and pointers to the two ADRs explaining why both exist), implementation guidance (API contracts, the specific chunking and retrieval-ranking configuration, and acceptance criteria per component), and a runbook (what to do if the PDF-ingestion pipeline fails, what the monitoring dashboard's key metrics mean, escalation contact for the original architect during a defined 30-day transition window). The platform team walks through a live incident-response drill using only the runbook before the architect signs off that handoff is complete.
> - *Monitoring.* The SLA agreed with the claims-ops director specifies: p95 retrieval latency under 4 seconds, a quality bar of ≥88% pass rate on a golden-set eval re-run monthly, and a defined escalation path (platform team on-call, then the original architect) if either threshold is missed for two consecutive weeks. A feedback register, reviewed biweekly with a rotating adjuster representative, logs real usage friction as it happens instead of waiting for a formal complaint.
> - *Iteration.* Four months in, the feedback register shows adjusters repeatedly requesting the system also flag potential fraud indicators — a capability outside the original discovery brief's scope (and one legal flagged as needing its own compliance review, echoing D5). The architect correctly routes this back to a fresh discovery pass rather than quietly bolting fraud-flagging onto the existing retrieval system, because it's a materially new use case with its own risk profile, not a tweak to the existing one.
> - *Result.* At every phase, the artifact that mattered wasn't a diagram — it was a decision or a document that let someone *other than the architect* understand, agree to, or act on the system. That's the throughline the whole domain is testing.
