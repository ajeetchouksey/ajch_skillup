...
> **Scenario.** A regional hospital network wants to deploy a Claude-powered clinical-intake assistant: patients describe symptoms in natural language, the system drafts a structured intake summary and a preliminary triage-urgency flag for the attending nurse. Walk the governance design.
>
> **Reasoning it through:**
> - *Step 1 — compliance gate, first, before any architecture.* This system touches PHI. Before any design work is finalized, procurement/legal must have a signed BAA with the model provider in place — no BAA, no PHI processing, full stop. This is a prerequisite, not a parallel workstream.
> - *Step 2 — name the specific risks.* Hallucination (a fabricated symptom detail changing the triage flag), bias (a triage-urgency model trained/prompted in a way that systematically under-flags urgency for certain patient demographics — a known, documented failure pattern in clinical AI), over-reliance (a busy nurse trusting the auto-generated urgency flag without reading the underlying symptom description), and data leakage (a multi-tenant deployment where one patient's intake context bleeds into another's session).
> - *Step 3 — map each risk to a structural control.* Hallucination → output-side grounding check requiring every symptom claim in the summary to be traceable to the patient's actual input text, not inferred. Bias → disaggregated evaluation of triage-flag accuracy broken out by age, sex, and other relevant subgroup before launch, not just aggregate accuracy — because a triage system with a demographic under-flagging pattern is both an ethical failure and a patient-safety failure simultaneously. Over-reliance → the interface design itself, not just policy: the system displays the *source* symptom text alongside the flag so the nurse's fastest path is reading the evidence, not just the label. Data leakage → hard per-session context isolation, verified in testing, not just assumed from the framework's default behavior.
> - *Step 4 — place the HITL gate.* Consequence severity is high (a missed urgent case is a patient-safety incident) and the action space is well-defined (a nurse reviews and confirms triage priority before it drives scheduling) — this is squarely an **approve-before-action gate**, not full autonomy and not lightweight post-hoc sampling. Critically, the interface is designed so review is fast and evidence-forward specifically to counter automation bias — a nurse under time pressure needs the *source text* visible, not just a green checkmark to click through.
> - *Step 5 — transparency, built in from the start.* Every intake summary logs the source patient text, the specific reasoning that drove the urgency flag, and the nurse's confirm/override decision — because if a triage decision is later challenged (a missed urgent case, a complaint, a regulatory inquiry), the system must be able to reconstruct *why* the flag was set, not just *what* the flag was.
> - *Result.* The architecture that emerges is not one big "governance module" bolted on — it's five decisions (BAA-first, risk-specific controls, disaggregated fairness evaluation, a rightly-placed approve-gate, and built-in transparency logging) made *during* design, each traceable to a specific named risk.
