...
**Layer your guardrails — no single control catches every failure mode, and a prompted rule is a request, not an enforcement mechanism.**
...
No single layer is sufficient alone: a system-prompt instruction ("never approve claims over $10,000") is a request, not an enforcement mechanism, unless the tool layer also enforces it structurally (a hard-coded ceiling on the claims-approval tool itself). **Prompted behavior is a strong default; structurally enforced behavior is the actual guarantee** — that's the exam's core distinction here.
...
**What breaks without this**: The $50,000 ceiling was a request to the model, not a constraint enforced by the tool itself, and the transfer tool had no independent cap — a financial-services team ships an agent instructed via system prompt to "never approve wire transfers over $50,000 without human review," a prompt-injection payload embedded in a forwarded email convinces the agent it has verbal authorization to bypass the review, and it calls the wire-transfer tool directly.
...
**Name the specific failure mode — you can't design a control for a risk you've only described generically.**

The recurring failure modes an exam-level architect is expected to identify by name:
...
An unmapped risk is not governed, it's just documented — the discipline is mapping each identified risk to a specific control from the guardrail stack, not leaving it as an acknowledged-but-unaddressed line item in a risk register.

### In Practice

**What breaks without this**: Because there was no output-side grounding check requiring every dosage figure to be traceable to a source document, and no explicit human sign-off gate for anything touching medication instructions specifically, the clinical-summarization assistant hallucinates a medication dosage in a discharge summary that a rushed nurse doesn't catch — a healthcare organization's risk assessment had listed "hallucination" as a generic risk with the mitigation "the model is generally reliable."
...
**Tier human review by consequence severity × reversibility — never apply one policy uniformly across action types.**

Common HITL patterns, from lightest to heaviest:
...
Over-applying HITL uniformly (gating every single output, including trivial ones) is itself a design failure — it produces automation bias (reviewers stop reading carefully because most gates are trivial) and destroys the throughput benefit the system was built for.
...
**What breaks without this**: Reviewers, overwhelmed by the volume of trivial renewal confirmations needing sign-off, develop automation bias and start rubber-stamping everything — including the rare claims-denial case that actually needed careful scrutiny — when an insurer applies the same "human reviews every output" HITL policy uniformly across both routine policy-renewal confirmations (thousands per day, trivial, reversible) and claims-denial decisions (rare, high-consequence, hard to reverse once communicated).
...
**GDPR, HIPAA, and FedRAMP each demand a different, non-interchangeable mechanism — treating "compliance" as one generic checkbox is the trap.**
...
Compliance requirements constrain the design space before a single line of prompt or code is written — data residency, retention, access scoping, and vendor agreements are inputs to architecture, not a post-hoc audit checklist applied to a finished system.

### In Practice

**What breaks without this**: A six-week engineering effort gets thrown out because the compliance gate should have been the first step, not a parallel-track legal task running behind the build — a healthcare startup builds and demos a Claude-powered clinical documentation assistant against real (de-identification assumed but not verified) patient data, then discovers post-demo that no BAA was ever signed with the model provider, and now has to treat the entire pilot as a potential HIPAA violation requiring breach assessment.
...
**Bias, fairness, and transparency are measured properties, not aspirational values — specify what's measured, how, and what happens at the threshold.**
...
Bias/fairness failures are a specific risk that gets caught by disaggregated evaluation (D4's mixed-methodology datasets, extended to subgroup-labeled test data) and mitigated by the same guardrail and HITL layers already discussed — **ethical AI isn't a separate control stack, it's a specific lens applied to the same one**.

### In Practice

**What breaks without this**: A disparity that was mathematically present in the training/retrieval data from day one stayed invisible in the aggregate accuracy number the team actually looked at, because no subgroup-disaggregated evaluation was ever run before launch — a bank's Claude-powered loan-pre-screening assistant passes evaluation at 91% aggregate accuracy and ships, then six months later a regulator's fair-lending audit finds the false-decline rate is nearly double for applicants from a specific zip-code cluster that correlates strongly with a protected demographic.
