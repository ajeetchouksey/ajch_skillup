...
**What breaks without this**: treating "multi-agent" as a single undifferentiated category leads a team to apply parallel-execution controls (isolation, conflict detection) to a sequential pipeline where the actual risk is a broken handoff, while a genuinely parallel setup ships with no scope-overlap check at all because nobody classified it as parallel in the first place.
...
Parallel agent sessions stay safe to run concurrently because each one operates against its own isolated slice of state, not because the agents happen not to collide. In practice this means each session works on its **own branch**, in its **own ephemeral, sandboxed environment** — its own checkout, its own dependency install, its own execution context — so one session's in-progress edits are never visible to another session mid-run, and a session that fails or produces a bad result can be discarded without any effect on a sibling session's branch or environment. Isolation is what makes "run three coding-agent sessions on three different issues at the same time" a safe default rather than a gamble: each session's world is a copy, not a shared reference, and the only point where their work can actually collide is the shared trunk each branch eventually targets — which is exactly why conflict detection has to run at merge time, not assumed away by isolation alone. Isolation prevents one session's *in-flight* state from corrupting another's; it does not prevent two *finished* sessions from proposing incompatible changes to the same underlying code.

### In Practice

**What breaks without this**: two agent sessions sharing a single working directory or branch — instead of each getting its own — means one session's uncommitted edits become invisible interference in the other's context: file contents change out from under a session mid-tool-call, and the resulting failure looks like a random flaky error with no obvious cause, because the actual cause (a second agent editing the same files concurrently) never appears in either session's own log.
...
### Key Concept
**Conflict detection and resolution — overlapping changes, duplicated effort, contradictory outputs**

Isolation keeps parallel work from interfering mid-flight; it doesn't guarantee the finished results are compatible. Three distinct conflict types show up once parallel branches converge, and each needs a different check to catch. **Overlapping changes** are the simplest and most mechanically detectable: two sessions edited the same file or the same lines, and a merge or rebase surfaces a literal conflict marker — this is the one that tooling catches automatically, the same way a standard PR merge would. **Duplicated effort** is subtler and not caught by any merge tool: two sessions solved the same underlying problem independently, in different files or different ways, because their scopes were assumed independent but actually overlapped in intent rather than in lines of code — nothing conflicts mechanically, but the codebase ends up with two competing implementations of the same thing. **Contradictory outputs** are subtler still: two sessions each produced a technically valid, individually defensible result, but the two results are mutually inconsistent when combined — one session standardizes an API to return `null` on a missing record, a second session standardizes the same API to throw instead, and both PRs pass their own tests in isolation while directly contradicting each other. Overlapping changes are caught by merge tooling; duplicated effort and contradictory outputs require a reviewer (or a coordinator agent) actually reading the set of parallel outputs together, the same qualitative-judgment step D4 requires for a single PR, applied across a batch instead of one at a time.

### In Practice

**What breaks without this**: a team that only checks for merge conflicts — the mechanically detectable case — approves two parallel PRs that each individually pass CI but jointly leave the codebase with two different, contradictory conventions for the same behavior; nothing in either PR's own evaluation (D4) catches it, because each PR's tests only checked its own change, never the other PR's.
...
A multi-agent system is observable when its record survives being examined *after the fact*, by someone who wasn't watching it run live — that's the bar "audit-ready" sets, as distinct from just "logged somewhere." An audit-ready artifact for a multi-agent workflow captures, per agent session: what it was assigned (the subtask or issue), what it decided and why (the plan, D1), what it actually did (the tool trace, D4), what it produced (diff, commit, PR), and — specific to the multi-agent case — which other agent or coordinator it received input from and which one it handed output to. This last piece is what turns a set of individually-logged sessions into a reconstructable workflow: without it, an auditor can see that five agents ran and produced five outputs, but not the shape of how those outputs actually depended on each other, which is exactly the information needed to explain a downstream failure that only manifests once outputs are combined.

### In Practice

**What breaks without this**: a multi-agent incident review that has each session's individual log but no record of which session's output fed which other session's input is left reconstructing the workflow topology from timestamps and guesswork — and if two sessions ran close enough in time, that reconstruction is unreliable exactly when it matters most, during an incident.
...
Beyond raw logs, a multi-agent workflow needs an explicit record of two specific moments: a **decision point** (why the coordinator routed a subtask to this agent rather than another, or why a given approach was chosen over an alternative the agent considered) and a **handoff** (the exact state — files, plan, context — that moved from one agent to the next, and confirmation the receiving agent actually picked it up correctly rather than silently reinterpreting it). Decision documentation matters because a multi-agent failure often traces back not to any single agent's execution but to the routing choice itself — the wrong subtask went to the wrong agent, or an approach was picked without the alternative ever being recorded, so nobody downstream can tell whether it was actually considered and rejected or just never came up. Handoff documentation matters because this is the exact seam where D4's context/environment failure category shows up at the multi-agent level: an executor agent starting from a plan the planning agent produced, but reading a stale or partial copy of it, fails for a reason invisible in the executor's own log — the executor did everything correctly with what it was actually given, and the given input was already wrong.

### In Practice

**What breaks without this**: without a record of the handoff itself — not just each agent's individual output — a downstream failure caused by an executor picking up an outdated version of a plan looks, from the evidence available, identical to an executor that reasoned incorrectly from a correct plan; the two require completely different fixes (D4), and without handoff documentation there's no way to tell which one actually happened.
...
**What breaks without this**: a monitoring setup built entirely around "did this agent throw an error" misses both failure modes here completely — a stall produces no error to catch, and degraded behavior produces valid-looking outputs at every step, so a workflow can run for days silently getting worse or silently stuck without a single alert firing.
...
Once a multi-agent failure is detected, the response has to match its severity and how well the root cause is understood, the same fix-matches-diagnosis discipline D4 applies to single-session failures. **Rollback** reverts the workflow (or the affected branch/agent's output) to the last known-good state — appropriate when the failure is clearly isolated to identifiable output that can be safely discarded without losing unrelated, still-valid work from sibling agents; isolation (covered above) is what makes a clean rollback of one agent's branch possible without disturbing others. **Human-in-the-loop** pauses the workflow and routes the decision to a person, appropriate when the root cause spans multiple agents or a coordinator-level decomposition choice, when the fix isn't mechanically obvious from the evidence, or when the failure is severe enough that an automated retry risks repeating it. The two aren't mutually exclusive stages of severity so much as different answers to "is the fix well-understood enough to apply automatically" — a clearly-isolated bad output rolls back and retries without needing a person to look at it; a workflow-level degradation with an unclear cause needs a person to actually diagnose it (D4's evidence trail, applied at the workflow level) before anything resumes.

### In Practice

**What breaks without this**: a workflow that automatically retries every failure, including ones where the root cause is a bad coordinator decomposition rather than a bad individual agent output, just re-runs the same flawed decomposition and produces the same degraded result again — automated recovery only works when the failure is actually the kind automation can fix, and a coordinator-level failure isn't.
...
### Key Concept
**Adding and updating agents without disrupting active work**

Multi-agent systems aren't static — a new specialized agent gets added, an existing agent's instructions or tool access get updated, and this has to happen without corrupting workflows that are already mid-run. The core discipline is the same isolation principle used for parallel execution, applied to configuration changes instead of concurrent branches: an in-flight session should keep running against the agent configuration it started with, and a configuration change should apply to *new* sessions going forward rather than mutating a session that's already partway through a plan built against the old configuration. Updating an agent's instructions mid-session — the same failure mode D3 warns about for context going stale mid-execution — means the plan the session already committed to (D1) no longer matches the rules it's now operating under, and the resulting output can't be evaluated against either the old rules or the new ones cleanly. The safe pattern versions agent configuration the same way code is versioned: an in-flight session pins to the version it started with, and a change takes effect for sessions that start after it lands.

### In Practice

**What breaks without this**: updating a shared agent's instructions while three sessions are already mid-execution against the old instructions means those three sessions' outputs get evaluated against criteria they were never actually operating under, and root-cause analysis (D4) on any of them is unreliable — the evidence trail shows reasoning consistent with instructions that, by the time anyone reviews the trail, no longer exist anywhere to compare against.
...
Removing an agent from a workflow — because it's been superseded, consolidated into a broader agent, or simply no longer needed — has to preserve the audit trail for every workflow that agent already participated in, even after it stops running new sessions. Retiring an agent means it no longer accepts new work, not that its history disappears: the decision and handoff records (above) that reference it, the sessions it completed, and the artifacts those sessions produced all need to remain queryable, because a post-hoc analysis of a workflow from six months ago that included a since-retired agent still has to be reconstructable. This is the multi-agent-lifecycle version of D3's memory-retention discipline — retiring the agent is a change to what's active going forward, not a deletion of what already happened, and conflating the two is how a team loses the ability to explain a past incident simply because the agent involved in it no longer exists in the current configuration.

### In Practice

**What breaks without this**: deleting a retired agent's configuration and historical session records together, rather than just removing it from future routing, means any post-hoc analysis (above) of a workflow that agent participated in becomes permanently impossible — the retirement decision quietly also destroyed the evidence trail needed to ever audit that workflow again.
...
> **Scenario.** A platform team is migrating an internal service's error-response format from a mixed ad-hoc style to a consistent structured schema. The work is split across a `payments`, `notifications`, and `auth` module, and a coordinator agent decomposes the migration into three parallel subtasks, one per module, each dispatched to its own Copilot coding agent session on its own branch, in its own isolated ephemeral environment.
>
> **Isolation holds; conflict detection catches what isolation can't.** All three sessions run concurrently without interfering — separate branches, separate environments, no shared state mid-run. Each session finishes cleanly: `payments` and `notifications` both merge without a single overlapping line, so an automated merge-conflict check waves both through. But a reviewer running the parallel-output check (not just the merge check) notices something the merge tool couldn't: `payments` standardizes missing-record errors by returning `{ "error": null, "found": false }`, while `notifications` — working from the same original ad-hoc codebase, but reasoning independently — standardizes the same pattern by throwing a `NotFoundError` instead. Neither PR touches a line the other touches. Both pass their own tests. It's a textbook contradictory-output conflict: clean on every mechanical check, incompatible in practice, because the two sessions never had any way to know what the other had decided.
>
> **Root cause traces to the coordinator, not either session.** Pulling the decision record, the reviewer finds the actual gap: the coordinator's subtask description told each session *what* to standardize but never specified *the target shape* to standardize to — an omission at the decomposition layer, not a reasoning error in either individual session (D4's classification, applied at the workflow level). Each session made a locally reasonable choice given genuinely incomplete instructions.
>
> **Response: human-in-the-loop, not rollback.** Rolling back and re-running either session alone wouldn't fix anything — retried against the same underlying coordinator instructions, the same ambiguity reproduces. The reviewer pauses the third (`auth`) session, which hasn't finished yet, before it locks in a third independent guess. A human resolves the ambiguity directly: the target shape is `{ "error": { "code": "NOT_FOUND" } }`. The coordinator's subtask template is updated to include that shape explicitly for all three modules — a fix at the decomposition layer, matching where the evidence actually pointed.
>
> **Recovery and lifecycle stay clean.** `payments` and `notifications` are rolled back to before their error-handling changes and re-dispatched as new sessions against the corrected subtask description; the still-valid parts of each session's original diff (unrelated refactoring in the same PR) are preserved rather than discarded wholesale, because isolation meant each branch could be selectively reverted without touching the other. The `auth` session, paused before completion, simply resumes against the corrected instructions rather than needing a rollback at all. Every decision — the original ambiguity, who caught it, what was decided, which sessions were rolled back and why — is recorded against the workflow's audit artifact, so a post-hoc review months later can reconstruct the whole sequence without needing to re-run anything.
